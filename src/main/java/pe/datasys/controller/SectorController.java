...
public class SectorController {
    ...
    public ResponseEntity<List<SectorDTO>> findAll() {
        List<SectorDTO> lista = service.findAll().stream().map( this::convertToDto).toList();
        return new ResponseEntity<>(lista, HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public ResponseEntity<SectorDTO> findById(@PathVariable("id") Integer id) {
        SectorEntity obj = service.findById(id);
        return new ResponseEntity<>(convertToDto(obj), HttpStatus.OK);
    }

    @PostMapping("/paginate")
    public ResponseEntity<Page<SectorDTO>> paginate(@RequestBody PaginationModel pagination) {
        Page<SectorDTO> pag = service.paginate(pagination.getPageNumber(), pagination.getRowsPerPage(),
    ...
    public ResponseEntity<SectorDTO> save(@Valid @RequestBody SectorDTO dto) {
        SectorEntity obj = service.save(convertToEntity(dto));
    ...
    public ResponseEntity<SectorDTO> update(@Valid @PathVariable("id") Integer id, @RequestBody SectorDTO dto) {
        SectorEntity obj = service.update(convertToEntity(dto), id);
        return new ResponseEntity<>(convertToDto(obj), HttpStatus.OK);
    }
    ...
    private SectorDTO convertToDto(SectorEntity obj){
        return mapper.map(obj, SectorDTO.class);
    }

    private SectorEntity convertToEntity(SectorDTO dto){
        return mapper.map(dto, SectorEntity.class);
    }
}
